	...
	do{
		...
		switch(choice){
			...
			case 8:
			    ...
			    scanf("%d", &x);
			    printf("Nhap gia tri cua mang mot chieu:\n");
			    for (int n = 0; n < x; n++){
			        scanf("%d  ",&arr[n]);
			    }
				printf("Nhap chi so cot muon chen: ");
			    scanf("%d", &updateCol);
			    if (updateCol < 0 || updateCol > col){
			        printf("Chi so cot khong hop le!\n");
			        break;
			    }
			    for (int n = 0; n < row; n++)
			    {
			        for (int m = col - 1; m >= updateCol; m--)
			        {
			            number[n][m + 1] = number[n][m];
			        }
			        number[n][updateCol] = arr[n];
			    }
			    col++;
